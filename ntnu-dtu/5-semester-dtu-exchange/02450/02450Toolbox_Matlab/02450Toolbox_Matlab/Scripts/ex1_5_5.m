...
% Data-processing functions in matlab changed around version 2019b. I think
% this check will ensure the right behavior, but you get errors try to
% change the version string to e.g. 9.6, 9.7 or 9.9 in case I misread time
% where the code changed. 
if verLessThan('matlab','9.8')     % check versions < 9.8 = 2019b
    % Run old matlab processing code.    
    ...
    %messy_data = table2array(messy_data);
    % weight is given as strings like 3'234, this has to be fixed by
    % conversion:
    messy_data.weight = cellfun(@(s)double(string(s).replace("'", "")), messy_data.weight);
    messy_data.carname = [];
    data = table2array(messy_data);
...
imagesc(missing_idx); ylabel('Observations'); xlabel('Attributes');
