function dbplot(X, y, fun, varargin)
...
% ColorOrder = ColorOrder * 0 + linspace(0,1, size(ColorOrder,1))';
% ColorOrder = bone(4);
...
hold all;
for c = 1:C
    mo = {'MarkerFaceColor', ColorOrder(mod(c-1,Ncol)+1,:), 'LineWidth', 1};
    mspec = 'wo';
    if ~isempty(markers)
        mspec = markers{c};
        mo = {'LineWidth', 1};
    end
        plot(X(y==c-1,1), X(y==c-1,2), mspec, mo{:}, 'MarkerSize', ms);
end
colormap(cMap);
% cMap
% colormap(cMap*0 + mean(cMap,2) );
%%
axis xy;
